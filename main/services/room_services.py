...
class SeatService:
    ...
    def select_seats(self, room_id):

        s_list = Seat.objects.filter(room_id=room_id).\
            values_list('seat_id', 'seat_number', 'x_position', 'y_position', 'is_open', 'is_reserved', 'is_with_plug')

        seats = []
        for s in s_list:
            seats.append({'seat_id': s[0], 'seat_number': s[1], 'x_position': float(s[2]), 'y_position': float(s[3]),
                          'is_open': s[4], 'is_reserved': s[5], 'is_with_plug': s[6]})
        return seats
